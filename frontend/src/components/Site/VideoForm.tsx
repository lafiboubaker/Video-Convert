...
const VideoForm = () => {
  ...
  const defaultTranslations = {
    ...
    error: language === "en" ? "Please enter a valid video URL" :
           ...
           language === "ar" ? "Please enter a valid video URL" :
  ...
  const handleConvert = async (e: React.FormEvent) => {
    ...
    console.log(`Using API URL: ${config.getPythonApiUrl()}`);
    ...
    try {
      ...
      const response = await axios.get(`${config.getPythonApiUrl()}/convert`, {
      ...
      const fullDownloadUrl = response.data.download_url;
      ...
      if (axios.isAxiosError(error)) {
        errorMessage = error.response?.data?.error || error.message || "Server connection error";
      ...
      setStatus('error');
      toast.error(`Failed to convert video: ${errorMessage}`);
    }
  };
            ...
            <div className="grid grid-cols-1 gap-3 mb-4">
              ...
              <div className="flex items-center justify-between mt-2">
                <button
                  ...
                  className="flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-300"
